...
export const tribonacci = (signature, n) => {
  ...
  while (result.length < n) {
    ...
    result.push(result[length - 3] + result[length - 2] + result[length - 1])
    /**
     * could also do something like
     *
     *    result.push(result.slice(-3).reduce((acc, n) => acc + n))
     *
     * but I did the alternative since it would be faster (not creating an array each time) while being still easy
     * to read.
     */
  }
  return result
